...
object Convert4sWithArray extends App {
  ...
  val path2 = "parb" // json path in string
  ...
  implicit class Json4sBigDecimalHelper(json: JValue) {
    ...
    def arrayFix(path: String) = {
      ...
      implicit val formats = DefaultFormats
      val finalArray = json.customExtract[JValue](path).map(gotArray => {
        val list = Seq("account_value/num")
        val finalJValue = list.foldLeft(gotArray)((p, i) => {
          p.customExtract[String](i).map(hasValue => {
            decimalFixerProto(p, i, JDecimal(BigDecimal(hasValue)))
          }).getOrElse(p)
        })
      ...
      println(compact(render(finalArray)))
    }
